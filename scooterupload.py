...
# Table name
table_name = 'scooter_ev'  # Update to your table name
...
# Function to insert scooter details into the database
def insert_scooter_details(scooter_name, segment_id, segment_name, model_type, year, motor_type, battery_type, price, image_urls, cursor):
    try:
        cursor.execute(f"""
            SELECT 1 
            FROM {table_name}
        ...
        cursor.execute(f"""
            INSERT INTO {table_name} (scooter_name, segment_id, segment_name, model_type, year, motor_type, battery_type, price,
        ...
        cursor.commit()
        return "Scooter details inserted successfully."
...
def upload_scooter():
    ...
    required_fields = ['scooter_name', 'segment_id', 'segment_name', 'model_type', 'year', 'motor_type', 'battery_type', 'price', 'image_paths']
    ...
    try:
        # Updated Azure SQL Database connection with direct authentication
        conn = pyodbc.connect(
        ...
        result = insert_scooter_details(scooter_name, segment_id, segment_name, model_type, year, motor_type, battery_type, price, image_urls, cursor)

        if not result:
            return jsonify({"error": "Failed to insert scooter details"}), 500
